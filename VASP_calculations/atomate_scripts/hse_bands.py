...
def generate_material_workflows(json_file):
    ...
    for material_name, mp_id in materials_dict.items():
        try:
            # Construct CIF filename (assumes CIF files are in directory ./cifs/)
            ...
            bands_flow = HSEUniformBandStructureMaker().make(structure=material_structure)
            
            # Create full workflow - change settings as required
            full_flow =  update_user_incar_settings(bands_flow,{"NCORE":10,"PRECFOCK":"Accurate"})            
